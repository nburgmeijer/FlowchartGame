...
class TextRenderer:
    ...
    def draw(
        ...
        render_copy(self.renderer, entry.texture, None, rect)
...
def render_copy(
    ...
    dst_f = None
    if src is not None:
        src_f = _sdl3.SDL_FRect(float(src.x), float(src.y), float(src.w), float(src.h))
    if dst is not None:
        dst_f = _sdl3.SDL_FRect(float(dst.x), float(dst.y), float(dst.w), float(dst.h))
